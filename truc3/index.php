  ...
  $('#generatePdfBtn').click(function() {
    ...
    $.ajax({
      ...
      success: function(dataFromDatabase) {
        ...
        var columns = ["ID", "Nom" , "Prenom" , "Login" ,"Telephone"];
        var rowCount = dataFromDatabase.length;
        var columnCount = columns.length;
        
        // Dessiner les en-têtes de colonne
        var startX = 10; // Position de départ en X
var startY = 10; // Position de départ en Y
var cellWidth = 40; // Largeur de chaque cellule
var cellHeight = 10; // Hauteur de chaque cellule
var cellSpacingX = 0; // Espacement horizontal entre les cellules
var cellSpacingY = 0; // Espacement vertical entre les cellules

// Dessiner les en-têtes de colonne
for (var j = 0; j < columnCount; j++) {
  var columnWidth = Math.max(cellWidth, doc.getStringUnitWidth(columns[j]) * 5); // Calcul de la largeur de la colonne en fonction de la largeur du texte
...
for (var i = 0; i < rowCount; i++) {
  var rowData = [dataFromDatabase[i].idU.toString(), dataFromDatabase[i].nom.toString(), dataFromDatabase[i].prenom.toString(), dataFromDatabase[i].login.toString(), dataFromDatabase[i].telephone.toString()]; // Convertir les valeurs en chaînes de caractères
  for (var j = 0; j < columnCount; j++) {
    var columnWidth = Math.max(cellWidth, doc.getStringUnitWidth(columns[j]) * 5); // Calcul de la largeur de la colonne en fonction de la largeur du texte
    doc.rect(startX + (columnWidth + cellSpacingX) * j, startY + cellHeight + (cellHeight + cellSpacingY) * i, columnWidth, cellHeight, 'S');
    doc.text(rowData[j], startX + (columnWidth + cellSpacingX) * j + columnWidth / 2 - doc.getStringUnitWidth(rowData[j]) * 5 / 2, startY + cellHeight + (cellHeight + cellSpacingY) * i + cellHeight / 2 + 2);
  }
}
